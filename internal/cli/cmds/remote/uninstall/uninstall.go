...
will be automatically loaded and command line flags will be ignored.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if validationErr := tls.ValidateInstallSSLInput(&sslArgs); validationErr != nil {
			...
			script := tls.GetUninstallScript()
			editedScript := tls.EditUninstallScriptWithFlags(script, &sslArgs)
